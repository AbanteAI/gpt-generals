...
class PlayerTUI:
            ...
            if key == ord("q") or key == ord("Q"):
                ...
                if success:
                    ...
                    if coins_collected:
                        ...
                        self.message = f"Unit {self.active_unit} moved {self.controller.direction_map[direction_key]}"
                    
                    # Advance turn
                    self.game.next_turn()
                else:
                    self.message = f"Move failed. Unit {self.active_unit} cannot move {self.controller.direction_map[direction_key]}."
    ...
    def display_game(self):
        ...
        status = f"Status: {'PAUSED' if self.paused else 'Running'} | Coins: {len(self.game.coin_positions)}/{self.total_coins}"
        ...
        try:
            ...
            while True:
                ...
                if not self.game.coin_positions:
                    self.message = f"Congratulations! You've collected all coins in {self.game.current_turn} turns."
            ...
            self.stdscr.clear()
            end_message = "Game Over! All coins collected." if not self.game.coin_positions else "Game ended by player."
            stats = f"Turns played: {self.game.current_turn} | Coins collected: {self.total_coins - len(self.game.coin_positions)}/{self.total_coins}"
            
            mid_y = self.height // 2
            self.stdscr.addstr(mid_y - 1, (self.width - len(end_message)) // 2, end_message, curses.A_BOLD)
            self.stdscr.addstr(mid_y + 1, (self.width - len(stats)) // 2, stats)
