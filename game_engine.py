...
class GameEngine:
    ...
    def _place_water(self) -> None:
        """Place water cells randomly on the grid."""
        available_positions = [(x, y) for x in range(self.grid_size) 
    ...
    def get_state(self) -> GameState:
        ...
        Returns:
            A dictionary containing the grid, unit positions, resources collected, and game over status
        """
        return {
            "grid": [row[:] for row in self.grid],  # Create a deep copy of the grid
            "units": {unit: info.copy() for unit, info in self.units.items()},  # Create a deep copy of the units
